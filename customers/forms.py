...
class CustomerForm(forms.ModelForm):
    ...
    cpf = forms.CharField(max_length=255, label="", min_length=11, widget=forms.TextInput(
        attrs={'placeholder': 'Cpf', 'id': 'Cpf', 'class': 'inputForm'}))
    name = forms.CharField(max_length=255, min_length=4, label="", widget=forms.TextInput(
        attrs={'placeholder': 'Nome', 'id': 'Name', 'class': 'inputForm'}))
    lastname = forms.CharField(max_length=255, min_length=4, label="", widget=forms.TextInput(
        attrs={'placeholder': 'Sobrenome', 'id': 'Lastname', 'class': 'inputForm'}))
    address = forms.CharField(max_length=300, min_length=10, label="", widget=forms.TextInput(
        attrs={'placeholder': 'Endereco', 'id': 'Address', 'class': 'inputForm'}))
    city = forms.CharField(max_length=255, min_length=3, label="", widget=forms.TextInput(
        attrs={'placeholder': 'Cidade', 'id': 'City', 'class': 'inputForm'}))
    telephone = forms.CharField(max_length=255, min_length=12, label="", widget=forms.TextInput(
